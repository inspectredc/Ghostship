...
void FreeLookCameraUpdate(struct Camera* c) {
    ...
    if (!gPuppyCam.enabled || c->cutscene != 0) {
        ...
        stub_camera_3(c);
        sCButtonsPressed =
            find_c_buttons_pressed(sCButtonsPressed, gPlayer1Controller->buttonPressed, gPlayer1Controller->buttonDown);
    ...
    if (!gPuppyCam.enabled || c->cutscene != 0) {
        if (gCurrLevelNum != LEVEL_CASTLE) {
            ...
            if ((c->cutscene == 0 && (gPlayer1Controller->buttonDown & R_TRIG) &&
                ...
                (gCameraMovementFlags & CAM_MOVE_FIX_IN_PLACE) || (sMarioCamState->action) == ACT_GETTING_BLOWN) {
    ...
    if (c->cutscene != 0) {
        ...
        if (gMarioState->action ==
            ACT_ENTERING_STAR_DOOR) { // god this is stupid and the fact I have to continue doing this is testament to
                                      // the idiocy of the star door cutscene >:(
