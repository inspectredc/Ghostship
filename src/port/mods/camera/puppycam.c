...
static u8 gPCOptionStringsFR[][64] = {
    { NC_ANALOGUE_FR }, { NC_CAMX_FR }, { NC_CAMY_FR },   { NC_INVERTX_FR },
    { NC_INVERTY_FR },  { NC_CAMC_FR }, { NC_SCHEME_FR },
};
static u8 gPCOptionStringsDE[][64] = {
    { NC_ANALOGUE_DE }, { NC_CAMX_DE }, { NC_CAMY_DE },   { NC_INVERTX_DE },
    { NC_INVERTY_DE },  { NC_CAMC_DE }, { NC_SCHEME_DE },
};
static u8 gPCFlagStringsFR[][64] = {
    { OPTION_DISABLED_FR }, { OPTION_ENABLED_FR }, { OPTION_SCHEME1_FR }, { OPTION_SCHEME2_FR }, { OPTION_SCHEME3_FR }
};
static u8 gPCFlagStringsDE[][64] = {
    { OPTION_DISABLED_DE }, { OPTION_ENABLED_DE }, { OPTION_SCHEME1_DE }, { OPTION_SCHEME2_DE }, { OPTION_SCHEME3_DE }
};
static u8 gPCToggleStringsFR[][64] = {
    { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }
};
static u8 gPCToggleStringsDE[][64] = {
    { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }, { NC_ANALOGUE_EN }
};
// static u8 gPCToggleStringsFR[][64] = {{NC_BUTTON_FR}, {NC_BUTTON2_FR}, {NC_OPTION_FR}, {NC_HIGHLIGHT_L_FR},
// {NC_HIGHLIGHT_R_FR}}; static u8 gPCToggleStringsDE[][64] = {{NC_BUTTON_DE}, {NC_BUTTON2_DE}, {NC_OPTION_DE},
// {NC_HIGHLIGHT_L_DE}, {NC_HIGHLIGHT_R_DE}};
#endif
static u8 gPCOptionStringsEN[][64] = {
    { NC_ANALOGUE_EN }, { NC_CAMX_EN }, { NC_CAMY_EN },   { NC_INVERTX_EN },
    { NC_INVERTY_EN },  { NC_CAMC_EN }, { NC_SCHEME_EN },
};
static u8 gPCFlagStringsEN[][64] = {
    { OPTION_DISABLED_EN }, { OPTION_ENABLED_EN }, { OPTION_SCHEME1_EN }, { OPTION_SCHEME2_EN }, { OPTION_SCHEME3_EN }
};
static u8 gPCToggleStringsEN[][64] = {
    { NC_BUTTON_EN }, { NC_BUTTON2_EN }, { NC_OPTION_EN }, { NC_HIGHLIGHT_L }, { NC_HIGHLIGHT_R }
};
...
static const struct gPCOptionStruct {
    u8 gPCOptionName; // This is the position in the newcam_options text array. It doesn't have to directly correlate
                      // with its position in the struct
...
static const struct gPCOptionStruct gPCOptions[] = {
    ...
    { /*Option Name*/ 0, /*Option Variable*/ &gPuppyCam.options.analogue, /*Option Value Text Start*/ 0,
      /*Option Minimum*/ FALSE, /*Option Maximum*/ TRUE },
    { /*Option Name*/ 6, /*Option Variable*/ &gPuppyCam.options.inputType, /*Option Value Text Start*/ 2,
      /*Option Minimum*/ 0, /*Option Maximum*/ 2 },
    { /*Option Name*/ 1, /*Option Variable*/ &gPuppyCam.options.sensitivityX, /*Option Value Text Start*/ 255,
      /*Option Minimum*/ 10, /*Option Maximum*/ 500 },
    { /*Option Name*/ 2, /*Option Variable*/ &gPuppyCam.options.sensitivityY, /*Option Value Text Start*/ 255,
      /*Option Minimum*/ 10, /*Option Maximum*/ 500 },
    { /*Option Name*/ 3, /*Option Variable*/ &gPuppyCam.options.invertX, /*Option Value Text Start*/ 0,
      /*Option Minimum*/ FALSE, /*Option Maximum*/ TRUE },
    { /*Option Name*/ 4, /*Option Variable*/ &gPuppyCam.options.invertY, /*Option Value Text Start*/ 0,
      /*Option Minimum*/ FALSE, /*Option Maximum*/ TRUE },
    { /*Option Name*/ 5, /*Option Variable*/ &gPuppyCam.options.turnAggression, /*Option Value Text Start*/ 255,
      /*Option Minimum*/ 0, /*Option Maximum*/ 100 },
};
...
static void puppycam_analogue_stick(void) {
    ...
    gPuppyCam.stick2[0] = -gPlayer1Controller->rawRightStickX;
    gPuppyCam.stick2[1] = gPlayer1Controller->rawRightStickY;
...
void puppycam_change_setting(s8 toggle) {
    ...
    // Forgive me father, for I have sinned. I guess if you wanted a selling point for a 21:9 monitor though, "I can
    // view this line in puppycam's code without scrolling!" can be added to it.
    *gPCOptions[gPCOptionSelected].gPCOptionVar =
        CLAMP(*gPCOptions[gPCOptionSelected].gPCOptionVar, gPCOptions[gPCOptionSelected].gPCOptionMin,
              gPCOptions[gPCOptionSelected].gPCOptionMax);
}
...
void puppycam_display_options() {
    ...
    for (i = 0; i < gPCOptionCap; i++) {
        ...
        if (scroll <= 140 && scroll > 32) {
            puppycam_print_text(160, scroll, (*gPCOptionStringsPtr)[gPCOptions[i].gPCOptionName],
                                gPCOptionSelected - i);
            if (gPCOptions[i].gPCOptionStart != 255) {
                ...
                puppycam_display_box(96, 111 + (32 * i) - (gPCOptionScroll * 32), 224,
                                     117 + (32 * i) - (gPCOptionScroll * 32), 0x80, 0x80, 0x80, 0xFF);
                maxvar = gPCOptions[i].gPCOptionMax - gPCOptions[i].gPCOptionMin;
                minvar = *gPCOptions[i].gPCOptionVar - gPCOptions[i].gPCOptionMin;
                puppycam_display_box(96, 111 + (32 * i) - (gPCOptionScroll * 32), 96 + (((f32)minvar / maxvar) * 128),
                                     117 + (32 * i) - (gPCOptionScroll * 32), 0xFF, 0xFF, 0xFF, 0xFF);
                puppycam_display_box(94 + (((f32)minvar / maxvar) * 128), 109 + (32 * i) - (gPCOptionScroll * 32),
                                     98 + (((f32)minvar / maxvar) * 128), 119 + (32 * i) - (gPCOptionScroll * 32), 0xFF,
                                     0x0, 0x0, 0xFF);
                gSPDisplayList(gDisplayListHead++, dl_ia_text_begin);
            }
        }
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, 255);
    print_generic_string(80 - newcam_sinpos, 132 - (32 * (gPCOptionSelected - gPCOptionScroll)),
                         (*gPCToggleStringsPtr)[3]);
    print_generic_string(232 + newcam_sinpos, 132 - (32 * (gPCOptionSelected - gPCOptionScroll)),
                         (*gPCToggleStringsPtr)[4]);
...
void puppycam_check_pause_buttons() {
    ...
    if (gPCOptionOpen) {
        if (ABS(gPlayer1Controller->rawStickY) > 60 || gPlayer1Controller->buttonDown & U_JPAD ||
        ...
        } else if (ABS(gPlayer1Controller->rawStickX) > 60 || gPlayer1Controller->buttonDown & L_JPAD ||
...
void puppycam_init(void) {
    ...
    if (gCurrLevelNum == 27 || (gCurrLevelNum == 36 && gCurrAreaIndex == 2) ||
        (gCurrLevelNum == 5 && gCurrAreaIndex == 2))
...
void puppycam_input_pitch(void) {
    ...
    if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PITCH_ROTATION) {
        ...
        // When Mario's moving, his pitch is clamped pretty aggressively, so this exists so you can shift your view up
        // and down momentarily at an actually usable range, rather than the otherwise baby range.
...
void puppycam_input_centre(void) {
    ...
    if (gPlayer1Controller->buttonPressed & inputDefault && gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_YAW_ROTATION &&
        !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_8DIR) && !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_4DIR) &&
...
static void puppycam_input_hold_preset1(f32 ivX) {
    ...
    } else if ((gPlayer1Controller->buttonDown & R_CBUTTONS && !gPuppyCam.options.analogue) ||
               gPuppyCam.stick2[0] != 0) {
        gPuppyCam.yawAcceleration += 50 * (gPuppyCam.options.sensitivityX / 100.f);
...
// Another alternative control scheme. This one aims to mimic the parallel camera scheme down to the last bit from the
// original game.
static void puppycam_input_hold_preset3(f32 ivX) {
    ...
    if (gPlayer1Controller->buttonPressed & U_CBUTTONS) {
        if ((gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_MED) && !(gMarioState->action & ACT_FLAG_AIR) &&
...
static void puppycam_input_hold(void) {
    ...
    // Analogue Camera stuff. If it fails to find an input, then it just sets stickmag to 100, which after calculations
    // means the value goes unchanged.
    ...
    if (!gPuppyCam.options.analogue) {
        switch (gPuppyCam.options.inputType) {
            default:
                ...
                break;
            case 1:
                puppycam_input_hold_preset2(ivX);
                puppycam_input_pitch();
                puppycam_input_zoom();
                puppycam_input_centre();
                break;
            case 2:
                puppycam_input_hold_preset3(ivX);
                puppycam_input_centre();
                break;
        }
    } else {
...
static void puppycam_input_press(void) {
    ...
    if ((gPlayer1Controller->buttonPressed & L_CBUTTONS && !gPuppyCam.options.analogue) ||
        (gPuppyCam.stickN[0] == 0 && gPuppyCam.stick2[0] < -DEADZONE)) {
    ...
    if ((gPlayer1Controller->buttonPressed & R_CBUTTONS && !gPuppyCam.options.analogue) ||
        (gPuppyCam.stickN[0] == 0 && gPuppyCam.stick2[0] > DEADZONE)) {
        gPuppyCam.stickN[0] = 1;
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_8DIR)
            gPuppyCam.yawTarget += 0x2000 * ivX;
...
static void puppycam_view_panning(void) {
    ...
    s32 panEx = (gPuppyCam.zoomTarget >= 1000) *
                250; // Removes the basic panning when idling if the zoom level is at the closest.
    f32 slideSpeed = 1;

    panMulti = CLAMP(gPuppyCam.zoom / (f32)gPuppyCam.zoomPoints[2], 0.f, 1.f);

    if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PANSHIFT && gMarioState->action != ACT_HOLDING_BOWSER &&
        gMarioState->action != ACT_SLEEPING && gMarioState->action != ACT_START_SLEEPING) {
        ...
        if (gMarioState->vel[1] == 0.0f) {
            panFloor = CLAMP(find_floor_height((s16)(gPuppyCam.targetObj->oPosX + expectedPanX),
                                               (s16)(gPuppyCam.targetObj->oPosY + 200),
                                               (s16)(gPuppyCam.targetObj->oPosZ + expectedPanZ)),
                             gPuppyCam.targetObj->oPosY - 50, gPuppyCam.targetObj->oPosY + 50);
...
static void puppycam_view_height_offset(void) {
    s16 floorTemp;
    s16 tempDist = sqrtf((gPuppyCam.pos[0] - gPuppyCam.focus[0]) * (gPuppyCam.pos[0] - gPuppyCam.focus[0]) +
                         (gPuppyCam.pos[1] - gPuppyCam.focus[1]) * (gPuppyCam.pos[1] - gPuppyCam.focus[1]) +
                         (gPuppyCam.pos[2] - gPuppyCam.focus[2]) * (gPuppyCam.pos[2] - gPuppyCam.focus[2]));

    floorTemp =
        find_floor_height(gPuppyCam.targetObj->oPosX, gPuppyCam.targetObj->oPosY + 50, gPuppyCam.targetObj->oPosZ);
    if (floorTemp > gPuppyCam.targetObj->oPosY - 50 && !(gMarioState->action & ACT_FLAG_SWIMMING_OR_FLYING)) {
        gPuppyCam.posHeight[0] =
            approach_f32_asymptotic(gPuppyCam.posHeight[0], floorTemp - gPuppyCam.targetFloorHeight, 0.05f);
    ...
    floorTemp =
        find_floor_height(gPuppyCam.targetObj->oPosX + LENSIN(tempDist, gPuppyCam.yaw), gPuppyCam.targetObj->oPosY + 50,
                          gPuppyCam.targetObj->oPosZ + LENCOS(tempDist, gPuppyCam.yaw));
    if (floorTemp > gPuppyCam.targetObj->oPosY - 50 && !(gMarioState->action & ACT_FLAG_SWIMMING_OR_FLYING) &&
        gPuppyCam.collisionDistance != gPuppyCam.zoomTarget) {
        gPuppyCam.posHeight[1] =
            approach_f32_asymptotic(gPuppyCam.posHeight[1], floorTemp - gPuppyCam.targetFloorHeight, 0.05f);
...
void find_surface_on_ray_cell(s16 cellX, s16 cellZ, Vec3f orig, Vec3f normalized_dir, f32 dir_length,
    ...
    if (cellX >= 0 && cellX <= (NUM_CELLS - 1) && cellZ >= 0 && cellZ <= (NUM_CELLS - 1)) {
        ...
        if (normalized_dir[1] > -0.99999f) {
            find_surface_on_ray_list(gStaticSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_CEILS].next, orig,
                                     normalized_dir, dir_length, hit_surface, hit_pos, max_length);
            find_surface_on_ray_list(gDynamicSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_CEILS].next, orig,
                                     normalized_dir, dir_length, hit_surface, hit_pos, max_length);
        }
        if (normalized_dir[1] < 0.99999f) {
            find_surface_on_ray_list(gStaticSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_FLOORS].next, orig,
                                     normalized_dir, dir_length, hit_surface, hit_pos, max_length);
            find_surface_on_ray_list(gDynamicSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_FLOORS].next, orig,
                                     normalized_dir, dir_length, hit_surface, hit_pos, max_length);
        }
        find_surface_on_ray_list(gStaticSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_WALLS].next, orig,
                                 normalized_dir, dir_length, hit_surface, hit_pos, max_length);
        find_surface_on_ray_list(gDynamicSurfacePartition[cellZ][cellX][SPATIAL_PARTITION_WALLS].next, orig,
                                 normalized_dir, dir_length, hit_surface, hit_pos, max_length);
    }
}
    ...
    *hit_surface = NULL;
    ...
    for (i = 0; i < step && *hit_surface == NULL; i++) {
        ...
        if ((cellPrevX != cellX) && (cellPrevZ != cellZ)) {
            find_surface_on_ray_cell(cellX, cellPrevZ, orig, normalized_dir, dir_length, hit_surface, hit_pos,
                                     &max_length);
            find_surface_on_ray_cell(cellPrevX, cellZ, orig, normalized_dir, dir_length, hit_surface, hit_pos,
                                     &max_length);
        }
    }
}
...
static s32 puppycam_check_volume_bounds(struct sPuppyVolume* volume, s32 index) {
    ...
    if (sPuppyVolumeStack[index]->shape == PUPPYVOLUME_SHAPE_BOX) {
        ...
        if (-sPuppyVolumeStack[index]->radius[1] < rel[1] && rel[1] < sPuppyVolumeStack[index]->radius[1] &&
            distCheck) {
            *volume = *sPuppyVolumeStack[index];
            return TRUE;
        }
...
void puppycam_projection_behaviours(void) {
    ...
    if (gPuppyCam.targetObj = gMarioState->marioObj) {
        if (gPuppyCam.options.turnAggression > 0 && gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_TURN_HELPER &&
            !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_8DIR) && gMarioState->vel[1] == 0.0f &&
            ...
            if (gPuppyCam.options.turnAggression > 0 || gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE) {
                ...
                // The deal here, is if Mario's moving, or he's sliding and the camera's within 90 degrees behind him,
                // it'll auto focus behind him, with an intensity based on the camera's centre speed. It also scales
                // with forward velocity, so it's a gradual effect as he speeds up.
                if ((ABS(gPlayer1Controller->rawStickX) > 20 &&
                     !(gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE)) ||
                    (gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE &&
                     (s16)ABS(((gPuppyCam.yaw + 0x8000) % 0xFFFF - 0x8000) -
                              ((gMarioState->faceAngle[1]) % 0xFFFF - 0x8000)) < 0x3000))
                    gPuppyCam.yawTarget =
                        gMarioState->faceAngle[1] + 0x8000 -
                        approach_s32((s16)(gMarioState->faceAngle[1] + 0x8000 - gPuppyCam.yawTarget), 0,
                                     ((gPuppyCam.options.turnAggression * 10) * ABS(gMarioState->forwardVel / 32) *
                                      ABS(gPlayer1Controller->rawStickX / 80.0f) * turnRate),
                                     ((gPuppyCam.options.turnAggression * 10) * ABS(gMarioState->forwardVel / 32) *
                                      ABS(gPlayer1Controller->rawStickX / 80.0f) * turnRate));
            }
        ...
        // This is the base floor height when stood on the ground. It's used to set a baseline for where the camera sits
        // while Mario remains a height from this point, so it keeps a consistent motion.
        gPuppyCam.targetFloorHeight =
            CLAMP(find_floor_height(gPuppyCam.targetObj->oPosX, gPuppyCam.targetObj->oPosY, gPuppyCam.targetObj->oPosZ),
                  gPuppyCam.targetObj->oPosY - 350, gPuppyCam.targetObj->oPosY + 300);

        if (gMarioState->vel[1] <= 0.0f)
            gPuppyCam.lastTargetFloorHeight =
                CLAMP(approach_f32_asymptotic(gPuppyCam.lastTargetFloorHeight, gPuppyCam.targetFloorHeight, 0.1f),
                      gPuppyCam.targetObj->oPosY - 350, gPuppyCam.targetObj->oPosY + 300);
        ...
        if (gMarioState->action == ACT_SLEEPING || gMarioState->action == ACT_START_SLEEPING)
            ...
            gPuppyCam.zoom = approach_f32_asymptotic(gPuppyCam.zoom,
                                                     MAX(gPuppyCam.zoomTarget / 1.5f, gPuppyCam.zoomPoints[0]), 0.2f);
        } else
        ...
        if (gMarioState->action & ACT_FLAG_MOVING || gMarioState->action & ACT_FLAG_AIR ||
            (gMarioState->action & ACT_FLAG_SWIMMING && !gMarioState->waterLevel - 100 - gMarioState->pos[1] > 5 &&
             gMarioState->forwardVel != 0.0f)) {
            // Clamp the height when moving. You can still look up and down to a reasonable degree but it readjusts
            // itself the second you let go.
            ...
            /*//Will tilt the camera just a wip thing though, doesn't work too well but will hopefully replace
            view_height_offset eventually. if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_HEIGHT_HELPER && gMarioState->floor
            && gMarioState->action & ACT_FLAG_MOVING) gPuppyCam.terrainPitch =
            approach_f32_asymptotic(gPuppyCam.terrainPitch, find_floor_slope(gMarioState, 0),
            (gMarioState->intendedMag/32)*0.2); else gPuppyCam.terrainPitch =
            approach_f32_asymptotic(gPuppyCam.terrainPitch, 0, 0.2);*/
        }

        // Applies a light outward zoom to the camera when moving. Sets it back to 0 when not moving.
        if (gMarioState->forwardVel > 0) {
            gPuppyCam.moveZoom = approach_f32(gPuppyCam.moveZoom, 100.0f * (gMarioState->forwardVel / 32.0f),
                                              gMarioState->forwardVel / 10, gMarioState->forwardVel / 10);
        } else {
        ...
        if (gPuppyCam.pitch > 0x38C0 && ABS(gPuppyCam.swimPitch) < 100) {
            gPuppyCam.zoom = approach_f32_asymptotic((f32)gPuppyCam.zoom, 250.0f,
                                                     CLAMP((f32)((gPuppyCam.pitch - 0x38C0) / 3072.0f), 0.0f, 1.0f));
        }
        ...
        if (!(gMarioState->action & ACT_FLAG_SWIMMING)) {
            ...
            if (gMarioState->waterLevel - 100 - gMarioState->pos[1] > 5 &&
                gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PITCH_ROTATION)
                gPuppyCam.swimPitch =
                    approach_f32_asymptotic(gPuppyCam.swimPitch, gMarioState->faceAngle[0] / 10, 0.05f);
            else
                gPuppyCam.swimPitch = approach_f32_asymptotic(gPuppyCam.swimPitch, 0, 0.2f);
        }
...
/// This is the highest level of the basic steps that go into the code. Anything above is called from these following
/// functions.
...
static void puppycam_projection(void) {
    ...
    gPuppyCam.yaw =
        gPuppyCam.yawTarget - approach_f32_asymptotic((s16)(gPuppyCam.yawTarget - gPuppyCam.yaw), 0, 0.3335f);
    gPuppyCam.pitch =
        gPuppyCam.pitchTarget - approach_f32_asymptotic((s16)(gPuppyCam.pitchTarget - gPuppyCam.pitch), 0, 0.3335f);
    // This adds the pitch effect when underwater, which is capped so it doesn't get out of control. If you're not
    // swimming, swimpitch is 0, so it's normal.
    ...
    if (gPuppyCam.targetObj) {
        ...
        if (gPuppyCam.targetObj2) {
            vec3s_set(targetPos2, gPuppyCam.targetObj2->oPosX, gPuppyCam.targetObj2->oPosY,
                      gPuppyCam.targetObj2->oPosZ);
            ...
            gPuppyCam.targetDist[0] = approach_f32_asymptotic(
                gPuppyCam.targetDist[0],
                (ABS(LENCOS(sqrtf(((targetPos[0] - targetPos2[0]) * (targetPos[0] - targetPos2[0])) +
                                  ((targetPos[2] - targetPos2[2]) * (targetPos[2] - targetPos2[2]))),
                            (s16)ABS(((gPuppyCam.yaw + 0x8000) % 0xFFFF - 0x8000) -
                                     (atan2s(targetPos[2] - targetPos2[2], targetPos[0] - targetPos2[0])) % 0xFFFF -
                                     0x8000) +
                                0x4000))),
                0.2f);
        } else {
        ...
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_X_MOVEMENT)
            gPuppyCam.focus[0] = targetPos3[0] + gPuppyCam.shake[0] +
                                 (gPuppyCam.pan[0] * gPuppyCam.targetDist[1] / gPuppyCam.zoomPoints[2]) * panD;
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_Y_MOVEMENT)
            gPuppyCam.focus[1] = targetPos3[1] + gPuppyCam.shake[1] +
                                 (gPuppyCam.pan[1] * gPuppyCam.targetDist[1] / gPuppyCam.zoomPoints[2]) +
                                 gPuppyCam.povHeight - gPuppyCam.floorY[0] + gPuppyCam.posHeight[0] +
                                 (gPuppyCam.swimPitch / 10);
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_Z_MOVEMENT)
            gPuppyCam.focus[2] = targetPos3[2] + gPuppyCam.shake[2] +
                                 (gPuppyCam.pan[2] * gPuppyCam.targetDist[1] / gPuppyCam.zoomPoints[2]) * panD;

        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_X_MOVEMENT)
            gPuppyCam.pos[0] = gPuppyCam.targetObj->oPosX +
                               LENSIN(LENSIN(gPuppyCam.targetDist[1], pitchTotal), gPuppyCam.yaw) + gPuppyCam.shake[0];
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_Y_MOVEMENT)
            gPuppyCam.pos[1] = gPuppyCam.targetObj->oPosY + gPuppyCam.povHeight +
                               LENCOS(gPuppyCam.targetDist[1], pitchTotal) + gPuppyCam.shake[1] - gPuppyCam.floorY[1] +
                               gPuppyCam.posHeight[1];
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_Z_MOVEMENT)
            gPuppyCam.pos[2] = gPuppyCam.targetObj->oPosZ +
                               LENCOS(LENSIN(gPuppyCam.targetDist[1], pitchTotal), gPuppyCam.yaw) + gPuppyCam.shake[2];
    }
}
...
static void puppycam_collision(void) {
    ...
    target[0][1] = gPuppyCam.targetObj->oPosY + (gPuppyCam.povHeight) -
                   CLAMP(gPuppyCam.targetObj->oPosY - gPuppyCam.targetFloorHeight, 0, 300);
    ...
    dist[0] = ((target[0][0] - hitpos[0][0]) * (target[0][0] - hitpos[0][0]) +
               (target[0][1] - hitpos[0][1]) * (target[0][1] - hitpos[0][1]) +
               (target[0][2] - hitpos[0][2]) * (target[0][2] - hitpos[0][2]));
    dist[1] = ((target[1][0] - hitpos[1][0]) * (target[1][0] - hitpos[1][0]) +
               (target[1][1] - hitpos[1][1]) * (target[1][1] - hitpos[1][1]) +
               (target[1][2] - hitpos[1][2]) * (target[1][2] - hitpos[1][2]));
...
static void puppycam_apply(void) {
    ...
    // Commented out simply because vanilla SM64 has this always set sometimes, and relies on certain camera modes to
    // apply secondary foci. Uncomment to have fun with certain angles.
